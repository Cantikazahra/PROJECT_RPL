                ...
                <form action="{{ route('user.pengajuan.process') }}" method="POST" class="space-y-3.5 text-left flex-grow flex flex-col justify-between">
                    ...
                    <div class="space-y-3 flex-grow overflow-y-auto no-scrollbar max-h-[430px] pr-0.5">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <div>
                                <label class="block text-[11px] font-bold text-gray-700 mb-1">Tahun Rencana Pembangunan</label>
                                <input type="number" name="tahun_pembangunan" value="{{ old('tahun_pembangunan') }}" 
                                    min="{{ date('Y') }}" 
                                    max="{{ date('Y') + 2 }}"
                                    placeholder="Contoh: {{ date('Y') }}" 
                                    class="w-full px-3 py-2 border border-gray-400 rounded-xl text-xs focus:outline-none focus:border-blue-500 bg-white text-black" 
                                    required>
                                <p class="text-[9px] text-gray-500 mt-1">Masukkan tahun rencana pembangunan (Maks. 2 tahun ke depan).</p>
                            </div>
                        </div>
